// Stage1.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
#define MAX_LIST_SIZE_PER_PROC 2147483648 // 268435456
...
void HyperCube_Class::check_list() {
	int tag = 0;
	unsigned int zero = 0;
	unsigned int creatingMax = (~zero) >> 1;
	int min_nbr = static_cast<unsigned int>(creatingMax); // creating maximum positive integer
...
// HyperCube Quicksort
//
bool HyperCube_Class::HyperCube_QuickSort() {
	int k; 			// Sub hypercube dimension
	int nbr_k; 			// Neighbor of this process along dim-k

	int32_t local_median;		// Median of local list
	int32_t pivot = 0;			// Value used to split local list 
	int idx;			// index where local list is split
	int list_size_le;		// Number of elements < pivot
	int list_size_gteq;		// Number of elements >= pivot
	int* nbr_list; 		// Sublist received from nbr process
	int nbr_list_size = 0;		// nbr_list size
	...
	for (k = dimension; k > 0; k--) {
		...
		//printf("[Proc: %0d] pivot = %d, local_median = %d, list_size = %d\n", my_id, pivot, local_median, list_size);
		//print_list();
		...
		list_size_gteq  = idx;
		list_size_le = list_size - idx;

		// Communicate with neighbor along dimension k
		nbr_k = neighbor_along_dim_k(k);

		// printf("[Proc: %0d] leq = %d,  gt = %d, nbr_k = %d\n", my_id, list_size_leq, list_size_gt, nbr_k);

		MPI_Status mpiStatus;


		if (nbr_k > my_id) {
			// MPI-2: Receive number of elements greater than pivot
			MPI_Recv(&nbr_list_size, 1, MPI_INT, nbr_k, 0, MPI_COMM_WORLD, &mpiStatus);
			// ***** Add MPI call here *****


			// MPI-3: Send number of elements less than or equal to pivot
			MPI_Send(&list_size_le, 1, MPI_INT, nbr_k, 0, MPI_COMM_WORLD);
			// ***** Add MPI call here *****
			...
			// MPI-4: Receive neighbor's list of elements that are greater than the pivot
			MPI_Recv(nbr_list, nbr_list_size, MPI_INT, nbr_k, 0, MPI_COMM_WORLD, &mpiStatus);
			// ***** Add MPI call here *****


			// MPI-5: Send list[idx ... list_size-1] to neighbor
			MPI_Send(&list[idx], list_size_le, MPI_INT, nbr_k, 0, MPI_COMM_WORLD);
			// ***** Add MPI call here *****


			// Merge local list of elements greater than equal to pivot with neighbor's list
			new_list = merged_list(&list[0], list_size_gteq, nbr_list, nbr_list_size);

			// Replace local list with new_list, update size
			delete[] list;
			delete[] nbr_list;
			list = new_list;
			list_size = list_size_gteq + nbr_list_size;
		}
		else {
			// MPI-6: Send number of elements greater than pivot
			MPI_Send(&list_size_gteq, 1, MPI_INT, nbr_k, 0, MPI_COMM_WORLD);
			// ***** Add MPI call here *****


			// MPI-7: Receive number of elements less than or equal to pivot
			MPI_Recv(&nbr_list_size, 1, MPI_INT, nbr_k, 0, MPI_COMM_WORLD, &mpiStatus);
			// ***** Add MPI call here *****
			...
			// MPI-8: Send list[0 ... idx - 1] to neighbor
			MPI_Send(&list[0], list_size_gteq, MPI_INT, nbr_k, 0, MPI_COMM_WORLD);
			// ***** Add MPI call here *****


			// MPI-9: Receive neighbor's list of elements that are less than or equal to pivot
			MPI_Recv(nbr_list, nbr_list_size, MPI_INT, nbr_k, 0, MPI_COMM_WORLD, &mpiStatus);
			// ***** Add MPI call here *****


			// Merge local list of elements less than pivot with neighbor's list
			new_list = merged_list(&list[idx], list_size_le, nbr_list, nbr_list_size);

			// Replace local list with new_list, update size
			delete[] list;
			delete[] nbr_list;
			list = new_list;
			list_size = list_size_le + nbr_list_size;
		}
...
int main(int argc, char* argv[])
{
	...
	// MPI
	MPI_Init(NULL, NULL); //MPI_Init(&argc, &argv);		// Initialize MPI
